...
async function api(
  ...
  if (useCronAuth) headers["Authorization"] = `Bearer ${CRON_KEY}`;
...
function toText(result: ApiResult): string {
  if (!result.ok) {
...
Returns: { nodes: [...], edges: [...] } compatível com bibliotecas de grafo`,
  ...
  async ({ funnel_key, status }) => {
    ...
    if (funnel_key) params.set("funnel_key", funnel_key);
