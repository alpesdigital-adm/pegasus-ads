  ...
  return (
    ...
    >
      ...
      <div className="px-5 -mt-12 relative z-10 pb-5 space-y-4">
        ...
        {/* Ações — Retry + Promover Controle */}
        <ActionPanel node={selectedNode} />
...
function ActionPanel({ node }: { node: GraphNode }) {
  ...
  const [retryFeedState, setRetryFeedState] = useState<"idle" | "loading" | "ok" | "err">("idle");
  const [retryStoriesState, setRetryStoriesState] = useState<"idle" | "loading" | "ok" | "err">("idle");
  const [promoteState, setPromoteState] = useState<"idle" | "loading" | "ok" | "err">("idle");

  const hasFeed    = !!node.feed_id;
  const hasStories = !!node.stories_id;
  const canPromote = node.kill_rule?.action === "promote" || node.kill_rule?.level === "L5";

  async function retry(placement: "feed" | "stories") {
    const creativeId = placement === "feed" ? node.feed_id : node.stories_id;
    if (!creativeId) return;

    const setState = placement === "feed" ? setRetryFeedState : setRetryStoriesState;
    setState("loading");
    try {
  ...
  async function promoteToControl() {
    const creativeId = node.feed_id || node.stories_id;
    if (!creativeId) return;

    setPromoteState("loading");
    try {
      const res = await fetch("/api/creatives/promote-control", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ creative_id: creativeId }),
      });
      if (res.ok) {
        ...
        setPromoteState("err");
        setTimeout(() => setPromoteState("idle"), 4000);
      }
    } catch {
      setPromoteState("err");
      setTimeout(() => setPromoteState("idle"), 4000);
  ...
  // Não mostrar painel se não há nenhuma ação disponível
  if (!hasFeed && !hasStories && !canPromote) return null;

  function btnLabel(state: typeof retryFeedState, idle: string) {
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {hasFeed && (
          <button
            onClick={() => retry("feed")}
            disabled={retryFeedState === "loading"}
            title="Regenerar criativo Feed com o mesmo prompt"
            className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all disabled:opacity-50 border ${
              retryFeedState === "ok"  ? "border-emerald-500/40 text-emerald-300 bg-emerald-900/20" :
              retryFeedState === "err" ? "border-red-500/40 text-red-300 bg-red-900/20" :
              "border-slate-600/40 text-slate-300 bg-slate-800/40 hover:bg-slate-700/50"
            }`}
          >
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
              className={retryFeedState === "loading" ? "animate-spin" : ""}>
              <path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8" />
              <path d="M21 3v5h-5" />
            </svg>
            {btnLabel(retryFeedState, "Retry Feed")}
          </button>
        )}
        ...
        {hasStories && (
          <button
            onClick={() => retry("stories")}
            disabled={retryStoriesState === "loading"}
            title="Regenerar criativo Stories com o mesmo prompt"
            className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all disabled:opacity-50 border ${
              retryStoriesState === "ok"  ? "border-emerald-500/40 text-emerald-300 bg-emerald-900/20" :
              retryStoriesState === "err" ? "border-red-500/40 text-red-300 bg-red-900/20" :
              "border-slate-600/40 text-slate-300 bg-slate-800/40 hover:bg-slate-700/50"
            }`}
          >
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
              className={retryStoriesState === "loading" ? "animate-spin" : ""}>
              <path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8" />
              <path d="M21 3v5h-5" />
            </svg>
            {btnLabel(retryStoriesState, "Retry Stories")}
          </button>
        )}

        {/* Promover a Controle — visível quando L5 (Winner Potencial) */}
        {canPromote && (
          <button
            onClick={promoteToControl}
            disabled={promoteState === "loading" || node.is_control}
            title={node.is_control ? "Este AD já é o controle" : "Promover este AD como novo baseline de controle"}
            className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all disabled:opacity-50 border ${
              node.is_control
                ? "border-emerald-500/40 text-emerald-400 bg-emerald-900/15 cursor-default"
                : promoteState === "ok"  ? "border-emerald-500/40 text-emerald-300 bg-emerald-900/20" :
                  promoteState === "err" ? "border-red-500/40 text-red-300 bg-red-900/20" :
                  "border-emerald-600/40 text-emerald-300 bg-emerald-900/15 hover:bg-emerald-900/25"
            }`}
          >
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
            </svg>
            {node.is_control ? "Controle ✓" : btnLabel(promoteState, "Promover Controle")}
          </button>
        )}
      </div>
    </div>
