/**
 * POST /api/adsets/archive
 *
 * Arquiva (deleta logicamente) um ou mais ad sets.
 *
 * Body (JSON):
 * {
...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      try {
        await metaFetch(
          `${META_BASE_URL}/${adsetId}`,
          {
            ...
            body: formBody({ status: "ARCHIVED", access_token: token }),
          }
        );
        results.push({ adset_id: adsetId, success: true });
        console.log(`[AdsetsArchive] Archived adset ${adsetId}`);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        results.push({ adset_id: adsetId, success: false, error: msg });
        console.error(`[AdsetsArchive] FAILED ${adsetId}:`, msg);
      }
    }
    ...
    console.error("[AdsetsArchive]", err);
    return NextResponse.json(
